# apps/bot/handlers/message_handler.py
"""
Handles direct text messages and commands sent to the bot by users.

Supports command parsing (/start, /help, /status) and plain
text messages forwarded by channel owners.

MUHIM: Bu fayl avval media_handler.py deb atalgan — noto'g'ri.
       Bu fayl faqat text message va commandlar bilan shug'ullanadi.
"""
...
def handle_direct_message(normalized: dict) -> None:
    """
    To'g'ridan-to'g'ri bot ga yuborilgan text message va commandlarni
    qayta ishlash.
    """
...
class DirectMessageHandler(BaseHandler):
    ...
    def handle(self) -> None:
        ...
        # If the message comes from a bot source context,
        # treat it as a text post creation.
